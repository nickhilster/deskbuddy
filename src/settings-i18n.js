...
(function initSettingsI18n(root) {
  const STRINGS = {
    en: {
      ...
      aboutCleanupConfirm: "Remove Clawd hooks/plugins from local agents and mark all agent integrations as uninstalled and disabled in Clawd? This does not delete agent apps or user configs.",
      ...
      mobileTitle: "Clawd on Mobile",
      mobileDeviceName: "Device name",
      mobileDesc: "Add this machine to the Clawd on Mobile app on your phone to monitor sessions from anywhere. Read-only LAN preview — no write or approval operations.",
      ...
      mobileQrHint: "Scan with your phone's camera to open Clawd on Mobile",
      mobileQrHintNative: "Scan with the Clawd Mobile app's built-in scanner",
      mobileClientPwa: "PWA (browser)",
      mobileClientNative: "Native App",
      mobileClientNativeNote: "Requires the Clawd Mobile app to already be installed — this link won't fall back to a browser.",
    ...
    zh: {
      ...
      aboutCleanupConfirm: "要移除本机各 agent 里的 Clawd hooks / plugins，并在 Clawd 里把所有 agent 集成标记为未安装且禁用吗？不会删除 agent 应用或用户配置。",
      ...
      mobileTitle: "Clawd on Mobile",
      mobileDeviceName: "设备名称",
      mobileDesc: "将这台设备添加到手机上的 Clawd on Mobile 应用，随时查看会话状态。只读局域网预览——无写入或审批操作。",
      ...
      mobileQrHint: "用手机相机扫描以打开 Clawd on Mobile",
      mobileQrHintNative: "用 Clawd Mobile 应用内置的扫描功能扫描",
      mobileClientPwa: "PWA（浏览器）",
      mobileClientNative: "原生应用",
      mobileClientNativeNote: "需要已安装 Clawd Mobile 应用——该链接不会回退到浏览器。",
    ...
    "zh-TW": {
      ...
      aboutCleanupConfirm: "要移除本機各 agent 內的 Clawd hooks / plugins，並在 Clawd 中將所有 agent 整合標記為未安裝且停用嗎？不會刪除 agent 應用程式或使用者設定。",
      ...
      mobileTitle: "Clawd on Mobile",
      mobileDeviceName: "裝置名稱",
      mobileDesc: "將這台裝置加入手機上的 Clawd on Mobile 應用程式，隨時查看工作階段狀態。唯讀區域網路預覽——無寫入或核准操作。",
      ...
      mobileQrHint: "用手機相機掃描以開啟 Clawd on Mobile",
      mobileQrHintNative: "用 Clawd Mobile 應用程式內建的掃描功能掃描",
      mobileClientPwa: "PWA（瀏覽器）",
      mobileClientNative: "原生應用程式",
      mobileClientNativeNote: "需要已安裝 Clawd Mobile 應用程式——此連結不會回退到瀏覽器。",
      mobileRegenerate: "重新生成令牌",
    ...
    ko: {
      ...
      aboutCleanupConfirm: "로컬 에이전트의 Clawd hooks/plugins를 제거하고 Clawd의 모든 에이전트 통합을 미설치 및 비활성 상태로 표시할까요? 에이전트 앱이나 사용자 설정은 삭제하지 않습니다.",
      ...
      mobileTitle: "Clawd on Mobile",
      mobileDeviceName: "기기 이름",
      mobileDesc: "이 기기를 휴대폰의 Clawd on Mobile 앱에 추가하여 어디서나 세션을 모니터링하세요. 읽기 전용 LAN 미리보기 — 쓰기 또는 승인 작업 불가.",
      ...
      mobileQrHint: "휴대폰 카메라로 스캔하여 Clawd on Mobile 열기",
      mobileQrHintNative: "Clawd Mobile 앱의 내장 스캐너로 스캔하세요",
      mobileClientPwa: "PWA (브라우저)",
      mobileClientNative: "네이티브 앱",
      mobileClientNativeNote: "Clawd Mobile 앱이 이미 설치되어 있어야 합니다 — 이 링크는 브라우저로 대체되지 않습니다.",
    ...
    ja: {
      ...
      aboutCleanupConfirm: "ローカル agent から Clawd hooks/plugins を削除し、Clawd のすべての agent 連携を未インストールかつ無効としてマークしますか？agent アプリやユーザー設定は削除しません。",
      ...
      mobileTitle: "Clawd on Mobile",
      mobileDeviceName: "デバイス名",
      mobileDesc: "このマシンをスマートフォンの Clawd on Mobile アプリに追加すると、どこからでもセッションを確認できます。読み取り専用 LAN プレビュー — 書き込みや承認操作は不可。",
      ...
      mobileQrHint: "スマートフォンのカメラでスキャンして Clawd on Mobile を開く",
      mobileQrHintNative: "Clawd Mobile アプリ内蔵のスキャナーでスキャン",
      mobileClientPwa: "PWA（ブラウザ）",
      mobileClientNative: "ネイティブアプリ",
      mobileClientNativeNote: "Clawd Mobile アプリがインストール済みである必要があります — このリンクはブラウザにフォールバックしません。",
