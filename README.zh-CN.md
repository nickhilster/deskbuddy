...
- **Pi** — 可选全局 extension，写入 `~/.pi/agent/extensions/deskbuddy`（从 Settings → Agents 安装，或执行 `npm run install:pi-extension`）；仅同步交互式 Pi 会话生命周期和工具活动状态，并保留 Pi 默认 YOLO 行为
...
普通用户建议直接从 **[GitHub Releases](https://github.com/nickhilster/deskbuddy/releases/latest)** 下载最新预构建安装包：

- **Windows**：`DeskBuddy-Setup-<version>-x64.exe` 或 `DeskBuddy-Setup-<version>-arm64.exe`
- **macOS**：`.dmg`
...
**Claude Code**、**Codex CLI** 会自动注册 hooks，开箱即用。**Copilot CLI**、**Gemini CLI**、**Antigravity CLI (agy)**、**Cursor Agent**、**CodeBuddy**、**Kiro CLI**、**Kimi Code CLI（Kimi-CLI）**、**Qwen Code**、**CodeWhale**、**opencode**、**Pi**、**OpenClaw**、**Hermes Agent**、**Qoder**、**QoderWork** 需要先在 **Settings → Agents** 安装对应集成；安装且启用后，Clawd 才会在启动时继续同步。也涵盖远程 SSH、WSL 及平台说明（macOS / Linux）：**[docs/guides/setup-guide.zh-CN.md](docs/guides/setup-guide.zh-CN.md)**
...
DeskBuddy 是一个社区驱动的项目。欢迎提 Bug、提需求、提 PR —— 在 [Issues](https://github.com/nickhilster/deskbuddy/issues) 里聊或直接提交 PR。
...
<table>
  <tr>
    ...
    <td align="center" valign="top" width="140"><a href="https://github.com/nickhilster"><img src="https://github.com/nickhilster.png" width="72" style="border-radius:50%" /><br /><sub><b>@nickhilster</b><br />维护者 · Teambotics</sub></a></td>
