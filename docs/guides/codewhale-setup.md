...
让 DeskBuddy（桌面宠物）能够感知并动画化 CodeWhale 的运行时状态。CodeWhale 是一个基于 DeepSeek API 的 AI 编程代理（TUI/CLI），运行在终端中。
...
早期实现把 `# managed by deskbuddy` 写在 `[[hooks.hooks]]` 前一行。`parseTomlSections()` 遇到 `[[hooks.hooks]]` 才开启新 section，导致 marker 被归到前一个 section，重复注册会不断追加 hook block，本地复现可见 `7 -> 8 -> 9 -> 10` 增长；卸载也可能只删 `6/7` 个 managed hook，留下孤儿 hook。
